 ...
 */
public class SymmetricConstrainedFeasibleAmoebaSearch {
    private double tolerance;
    private int maxIteration;
    private PrintStream printStream;
    private RandomData randomData;

    public SymmetricConstrainedFeasibleAmoebaSearch(double tolerance, int maxIteration, PrintStream printStream, SymmetricRationalizableFinder rationalizableFinder) {
    ...
    public SymmetricConstrainedFeasibleAmoebaSearch(double tolerance, int maxIteration, PrintStream printStream) {
        this(tolerance, maxIteration, printStream, new LpSolveSymmetricRationalizableFinder());

    }

    public SymmetricConstrainedFeasibleAmoebaSearch(double tolerance, int maxIteration) {
    ...
    public StrategySearchResult runForResult(SymmetricGame game, Strategy initialStrategy, Set<Action> restrictedActions) {
        return runForResult(game, initialStrategy, restrictedActions, false);
    }

    public Strategy run(Strategy initialStrategy, Set<Action> restrictedActions, boolean randomize, Action[] actions, PayoffMatrix pm) {
    ...
    public StrategySearchResult runForResult(Strategy initialStrategy, Set<Action> restrictedActions, boolean randomize, Action[] actions, PayoffMatrix pm) {
        ...
        loop:
        for (iteration = 1; SIMPLICES > 1; iteration++) {
    ...
    public Strategy run(SymmetricGame game, Strategy initialStrategy, Set<Action> restrictedActions, boolean randomize, Player[] players, Action[] actions) {
        return run(initialStrategy, restrictedActions, randomize, actions, createPayoffMatrix(game, players, actions));
    }

    public StrategySearchResult runForResult(SymmetricGame game, Strategy initialStrategy, Set<Action> restrictedActions, boolean randomize, Player[] players, Action[] actions) {
        return runForResult(initialStrategy, restrictedActions, randomize, actions, createPayoffMatrix(game, players, actions));
    }
    ...
    public StrategySearchResult runForResult(SymmetricGame game, Strategy initialStrategy, Set<Action> restrictedActions, boolean randomize) {
        Action[] actions = game.getActions().toArray(new Action[0]);
        Player[] players= game.players().toArray(new Player[0]);
        return runForResult(initialStrategy, restrictedActions, randomize, actions, createPayoffMatrix(game, players, actions));
    }
    ...
    private void generateFeasibleTowards(double[] newVertex, double[] first, double[] second, double scale, PayoffMatrix pm, int[] restricted, double[] distribution) {
